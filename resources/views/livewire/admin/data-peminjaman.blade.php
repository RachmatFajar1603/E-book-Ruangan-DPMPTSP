    ...
    <main class="p-4 sm:ml-64">
        <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-gray-600">
            ...
            <div class="bg-white p-4 rounded-md shadow-md flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl sm:text-3xl mt-2">{{ $sumverified }} <span class="text-sm sm:text-lg text-green-500">(0%)</span></p>
            ...
            <div class="bg-white p-4 rounded-md shadow-md flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl sm:text-3xl mt-2">{{ $sumrejected }} <span class="text-sm sm:text-lg text-red-500">(0%)</span></p>
        ...
        <div class="bg-white p-4 rounded-md shadow-md mt-8 overflow-x-auto">
                ...
                <table class="min-w-full border-collapse w-full">
                    <thead>
                        <tr>
                            ...
                            <th class="border-b px-4 py-2 text-left">NIP/NO REG/NIK</th>
                            <th class="border-b px-4 py-2 text-left">BIDANG</th>
                            <th class="border-b px-4 py-2 text-left">PENANGGUNG JAWAB</th>
                            <th class="border-b px-4 py-2 text-left">RUANGAN</th>
                            <th class="border-b px-4 py-2 text-left">TGL.MULAI PEMINJAMAN</th>
                            <th class="border-b px-4 py-2 text-left">TGL. SELESAI PEMINJAMAN</th>
                            <th class="border-b px-4 py-2 text-left">WAKTU MULAI</th>
                            <th class="border-b px-4 py-2 text-left">WAKTU SELESAI</th>
                            <th class="border-b px-4 py-2 text-left">KEPERLUAN</th>
                            <th class="border-b px-4 py-2 text-left">JUMLAH YANG HADIR</th>
                    ...
                    <tbody>
                        @foreach ($peminjaman as $item)
                            <tr class="@if ($item->status == 'booked') bg-blue-100 @elseif ($item->status == 'verified') bg-green-100 @else bg-red-100 @endif">
                                <td class="border-b px-4 py-2">{{ ($peminjaman->currentPage() - 1) * $peminjaman->perPage() + $loop->iteration }}</td>
                                ...
                                <td class="border-b px-4 py-2 @if ($item->status == 'booked') text-yellow-400 @elseif ($item->status == 'verified') text-green-400 @else text-red-400 @endif">
                                    {{ ucfirst($item->status) }}
                                </td>
                                @if (auth()->user()->can('approve_peminjaman') || auth()->user()->can('reject_peminjaman'))
                                    <td class="border-b px-4 py-2">
                                        <div class="flex flex-col items-start space-y-2">
                                            @if ($item->status == 'booked')
                                                <button wire:click="approve({{ $item->id }})" class="bg-green-500 text-white hover:bg-green-600 flex items-center px-2 py-1 rounded-lg">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                                    </svg>
                                                    <span class="ml-1">Approve</span>
                                                </button>
                                                <button wire:click="reject({{ $item->id }})" class="bg-red-500 text-white hover:bg-red-600 flex items-center px-2 py-1 rounded-lg">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            @elseif($editingId === $item->id)
                                                ...
                                                <button wire:click="saveEdit" class="bg-blue-500 text-white hover:bg-blue-600 flex items-center px-2 py-1 rounded-lg mt-2">
                                                    <span class="ml-1">Save</span>
                                                </button>
                                                <button wire:click="cancelEdit" class="bg-gray-500 text-white hover:bg-gray-600 flex items-center px-2 py-1 rounded-lg mt-2">
                                                    <span class="ml-1">Cancel</span>
                                                </button>
                                            @else
                                                <button wire:click="startEdit({{ $item->id }})" class="bg-yellow-500 text-white hover:bg-yellow-600 flex items-center px-2 py-1 rounded-lg">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mt-4">
                <span class="mb-2 sm:mb-0">Menampilkan {{ $peminjaman->firstItem() }} hingga {{ $peminjaman->lastItem() }} dari {{ $peminjaman->total() }} entri</span>
